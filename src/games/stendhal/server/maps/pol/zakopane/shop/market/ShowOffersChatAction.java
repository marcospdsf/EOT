 ...
 *                                                                         *
 ***************************************************************************/
package games.stendhal.server.maps.pol.zakopane.shop.market;
...
public class ShowOffersChatAction implements ChatAction {
	...
	public void fire(Player player, Sentence sentence, EventRaiser npc) {
		if (sentence.hasError()) {
			...
			npc.setCurrentState(ConversationStates.ATTENDING);
						...
						|| (sentence.getExpressions().iterator().next().toString().equals("pokaż"))){
	...
	private void handleSentence(Player player, Sentence sentence, EventRaiser npc) {
		...
		if (counter == 0) {
			String expiredAddition = onlyMyExpiredOffers ? "zakończonych " : "";
			player.sendPrivateText("Aktualnie brak " + expiredAddition + "ofert na rynku.");
		}
	}
	...
	private boolean checkForMineFilter(Sentence sentence) {
		for (Expression expression : sentence) {
			if ((expression.toString().equals("mine"))
				|| (expression.toString().equals("moje"))) {
				return true;
			}
		}
		return false;
	...
	private boolean checkForMyExpiredFilter(Sentence sentence) {
		for (Expression expression : sentence) {
			if ((expression.getNormalized().equals("expire"))
				|| (expression.getNormalized().equals("zakończ"))) {
				return true;
			}
		}
		return false;
