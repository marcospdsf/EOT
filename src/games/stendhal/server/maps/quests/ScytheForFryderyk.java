...
public class ScytheForFryderyk extends AbstractQuest {
	...
	private void createRequestingStep() {
		...
		fryderyk.add(ConversationStates.ATTENDING,
			...
			"Dziękuję za ofertę dobry człowieku, ale ta scythe wystarczy mi na pięć wiosen i nie potrzebuję ich więcej.",
			null);
		...
		fryderyk.add(
			...
			null,
			ConversationStates.QUEST_OFFERED,
			"Nie wiesz co to jest scythe?! Jedyne narzędzie, którym można skosić plony na polu. Składa się z metalowego ostrza i drewnianego kija, a wszystko przypomina literę L. Zrobisz to dla mnie?",
			null);
	}
	...
	private void createBringingStep() {
		...
		fryderyk.add(ConversationStates.IDLE,
			...
			"Hej! Czy ta scythe jest dla mnie?", null);

		fryderyk.add(ConversationStates.IDLE,
			ConversationPhrases.GREETING_MESSAGES,
			new AndCondition(new GreetingMatchesNameCondition(fryderyk.getName()),
					new QuestInStateCondition(QUEST_SLOT, "start"),
					new NotCondition(new PlayerHasItemWithHimCondition("scythe"))),
			ConversationStates.ATTENDING,
			"Hej mój przyjacielu. Pamintsz o kosie, którą mi obiecałeś. Pytałem się o nią wcześniej? Moje plony czekają na mnie...",
			null);
