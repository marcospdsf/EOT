/* $Id: RedStone.java,v 1.65 2012/11/18 03:53:53 tigertoes Exp $ */
/***************************************************************************
 ...
 * </ul>
 */
public class RedStone extends AbstractQuest {

	private static final int REQUIRED_WOOD = 100;
	
	private static final int REQUIRED_MINUTES = 240;

	private static final String QUEST_SLOT = "redstone";
	...
	public List<String> getHistory(final Player player) {
		...
		res.add("Spotkałem Miriri");
		final String questState = player.getQuest(QUEST_SLOT);
		if ("rejected".equals(questState)) {
			res.add("Nie chcę kamienia od Miriri");
			return res;
		}
		res.add("Chcę kamień od Miriri");
		if ((player.isEquipped("money", REQUIRED_WOOD)) || isCompleted(player)) {
			res.add("Musze przynieść pieniadze.");
		}
		if (isCompleted(player)) {
			res.add("Dałem Miriri pieniądze. Dostałem przepiękny czerwony kamień");
		}
		if(isRepeatable(player)){
			res.add("Mogę iść po nowy kamień do Miriri.");
		} 
		return res;
	}
	...
	private void prepareRequestingStep() {
		final SpeakerNPC npc = npcs.get("Miriri");
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Witaj znowu! Masz te 100 money o których wcześniej rozmawialiśmy?",
			null);
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Dobrze, że jesteś. Przynieś szybko te 100 money a dostaniesz czerwony kamień!",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
			...
			ConversationStates.ATTENDING,
			"Już się umówiliśmy, że dotarczysz pieniądze. 100 money, pamintsz?",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				"Mam przepiękny #czerwony #kamień. Moge ci go dać ale nie ma nic za darmo. Przynieś mi 100 money a dam ci kamień. Co ty na to ?",
				null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				ConversationStates.QUEST_OFFERED, 
				"Mam kolejny czerwony kamień. Dam ci go za kolejne 100 money. Pasuje?",
				null);
	...
	private void prepareBringingStep() {
		...
		reward.add(new EquipItemAction("czerwony kamień", 1));
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Czerwony kamień", 
				"Miriri da mi czerwony kamień za trochę złota.", 
				true);
