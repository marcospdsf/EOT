/* $Id: GoldenStone.java,v 1.65 2012/11/18 03:53:53 tigertoes Exp $ */
/***************************************************************************
 ...
 * </ul>
 */
public class GoldenStone extends AbstractQuest {

	private static final int REQUIRED_WOOD = 5000;
	
	private static final int REQUIRED_MINUTES = 720;

	private static final String QUEST_SLOT = "goldenstone";
	...
	public List<String> getHistory(final Player player) {
		...
		res.add("Spotkałem Jacer");
		final String questState = player.getQuest(QUEST_SLOT);
		if ("rejected".equals(questState)) {
			res.add("Nie chcę kamienia od Jacera");
			return res;
		}
		res.add("Chcę kamień od Jacera");
		if ((player.isEquipped("money", REQUIRED_WOOD)) || isCompleted(player)) {
			res.add("Musze przynieść pieniadze.");
		}
		if (isCompleted(player)) {
			res.add("Dałem Jacerowi pieniądze. Dostałem przepiękny złoty kamień");
		}
		if(isRepeatable(player)){
			res.add("Mogę iść po nowy kamień do Jacera.");
		} 
		return res;
	}
	...
	private void prepareRequestingStep() {
		final SpeakerNPC npc = npcs.get("Jacer");
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Witaj znowu! Masz te 5000 money o których wcześniej rozmawialiśmy?",
			null);
		...
		npc.add(ConversationStates.IDLE, 
			...
			"Dobrze, że jesteś. Przynieś szybko te 5000 money a dostaniesz złoty kamień!",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
			...
			ConversationStates.ATTENDING,
			"Już się umówiliśmy, że dotarczysz pieniądze. 5000 money, pamintsz?",
			null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				"Mam przepiękny #złoty #kamień. Moge ci go dać ale nie ma nic za darmo. Przynieś mi 5000 money a dam ci kamień. Co ty na to ?",
				null);
		...
		npc.add(ConversationStates.ATTENDING,
				...
				ConversationStates.QUEST_OFFERED, 
				"Mam kolejny złoty kamień. Dam ci go za kolejne 5000 money. Pasuje?",
				null);
	...
	private void prepareBringingStep() {
		...
		reward.add(new EquipItemAction("złoty kamień", 1));
	...
	public void addToWorld() {
		...
		fillQuestInfo(
				"Złoty kamień", 
				"Jacer da mi złoty kamień za trochę złota.", 
				true);
