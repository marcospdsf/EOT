...
public class obsmedal extends AbstractQuest {
	...
	private void step_3() {
			...
			new ChatAction() {
				...
				public void fire(final Player player, final Sentence sentence, final EventRaiser raiser) {
					...
					if (!missingSomething && (neededWoodLogs > 0)) {
						if (player.isEquipped("sapphireowy medal", neededWoodLogs)) {
							player.drop("sapphireowy medal", neededWoodLogs);
							neededWoodLogs = 0;
						} else {
							final int amount = player.getNumberOfEquipped("sapphireowy medal");
							if (amount > 0) {
								player.drop("sapphireowy medal", amount);
								neededWoodLogs -= amount;
							}

							raiser.say("Nie mam jak cię awansować bez "
								+ Grammar.quantityplnoun(neededWoodLogs, "sapphireowy medal","")
								+ ".");
							missingSomething = true;
						}
					}
		...
		npc.add(ConversationStates.IDLE, ConversationPhrases.GREETING_MESSAGES,
			...
			ConversationStates.IDLE, null, new ChatAction() {
				...
				public void fire(final Player player, final Sentence sentence, final EventRaiser raiser) {
					...
					player.setDefXP(150000 + player.getDefXP());
					
					player.incAtkXP();
					player.incDefXP();
					player.addKarma(500);
					final Item magicSword = SingletonRepository.getEntityManager().getItem("diamondowy medal");
			...
			new ChatAction() {
				...
				public void fire(final Player player, final Sentence sentence, final EventRaiser raiser) {
					...
					raiser.say("Będę potrzebował " + neededIron + " pazura arktycznego smoka, "
							+ neededWoodLogs + " sapphireowy medal "
							+ neededGoldBars + " mithril bar i "
		...
		npc.add(ConversationStates.ANY, "sapphireowy medal", null,
				ConversationStates.ATTENDING,
				"Przynieś sapphireowy medal aby udowodnić mi, że pomogłeś Trenerowi Magów.", null);
	...
	public List<String> getHistory(final Player player) {
			...
			if (isCompleted(player)) {
				res.add("Pomogłem Majorowi Klykhowi i trenerom w twierdzy Ados i dostałem diamondowe odznaczenie.");
			}
			return res;	
	}
