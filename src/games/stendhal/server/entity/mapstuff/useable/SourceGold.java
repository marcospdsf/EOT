/* $Id: SourceGold.java,v 1.4 2011/02/13 15:28:45 edi18028 Exp $ */
/***************************************************************************
...
/**
 * A gold source is a spot where a player can prospect for gold. He
...
public class SourceGold extends PlayerActivityEntity {
	...
	private double getSuccessProbability(final Player player) {
		...
		final int skiller = player.getMiningLevel();
		
		if (skiller != 0) {
			probability = Math.max(probability, skiller); 
					//Double.parseDouble(skill));
		}
	...
	protected boolean isPrepared(final Player player) {
		if (player.isEquipped(NEEDED_EQUIPMENT_1) && player.getMiningLevel() >= 12) {
			return true;
		} else if (player.isEquipped(NEEDED_EQUIPMENT_1) && player.getMiningLevel() < 12) {
			player.sendPrivateText("You need to have level 12 in mining to mine.");
			return false;
		}
