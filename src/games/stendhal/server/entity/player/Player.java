...
public class Player extends DressedEntity implements UseListener {
	...
	public static Player createZeroLevelPlayer(final String characterName,
		...
		for (final String slot : Arrays.asList("armor", "rhand", "head", "legs", "feet")) {
			player.addSlot(slot);
		}
		...
		entity = SingletonRepository.getEntityManager().getItem("leather helmet");
		slot = player.getSlot("head");
		slot.add(entity);
		
		entity = SingletonRepository.getEntityManager().getItem("leather legs");
		slot = player.getSlot("legs");
		slot.add(entity);
		
		entity = SingletonRepository.getEntityManager().getItem("leather boots");
		slot = player.getSlot("feet");
		slot.add(entity);
	 ...
	 */
	@Override
	public int getCappedMiningLevel() {
		// Blue line in https://sourceforge.net/p/arianne/feature-requests/1330/
		// reduced using median instead of average as reference
		return Math.min(this.miningLevel, getMaxMiningForLevel(level));
	}
