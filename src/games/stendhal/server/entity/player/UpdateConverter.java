...
public abstract class UpdateConverter {
	...
	private static final List<String> GLOVES_OLD_0_1 = Arrays.asList(
		      ...
		      "mithril gloves");
		  private static final List<String> GLOVES_NEW_0_2 = Arrays.asList(
				  "leather gloves","leather enchanced gloves","leather strong gloves",
			      "plate gloves","red gloves","blue gloves","fire gloves",
			      "ice gloves","golden gloves","shadow gloves","black gloves",
			      "mithril gloves");
	...
	public static String updateItemName(String name) {
		if (name != null) {
    		...
    		idx = GLOVES_OLD_0_1.indexOf(name);
    		if (idx != -1) {
    			name = GLOVES_NEW_0_2.get(idx);
    		}
		}
	...
	public static void updateKeyring(Player player) {
		if (player.getFeature("keyring") != null) {
			...
			if (!player.hasSlot("smallbag")){
				player.addSlot(new PlayerSlot ("smallbag"));
			}
			if (!player.hasSlot("holder")){
				player.addSlot(new PlayerSlot ("holder"));
			}
			/*
			...
			if (!player.equip("smallbag", scrollbag)){
				logger.error("Failed to place scrollbag in smallbag: " + player);
				return;
			}
			
			if (!player.equip("weaponbag", weaponbag)){
				logger.error("Failed to place weaponbag in holder: " + player);
				return;
			}

			RPSlot oldSlot = player.getSlot("keyring");
			RPSlot oldSlot2 = player.getSlot("scrollbag");
			RPSlot oldSlot3 = player.getSlot("weaponbag");
			
			EntitySlot newSlot = keyring.getEntitySlot("content");
			EntitySlot newSlot2 = scrollbag.getEntitySlot("content");
			EntitySlot newSlot3 = weaponbag.getEntitySlot("content");
			...
			if (!"scrollbag".equals(newSlot2.getContentSlotName())) {
				logger.error("Scrollbag has incorrect slot name: "
						+ newSlot2.getContentSlotName() + ", item is: " + scrollbag);
				return;
			}
			
			if (!"weaponbag".equals(newSlot3.getContentSlotName())) {
				logger.error("Weaponbag has incorrect slot name: "
						+ newSlot3.getContentSlotName() + ", item is: " + weaponbag);
				return;
			}
			
			ArrayList<RPObject> contents = new ArrayList<RPObject>(oldSlot.size());
			ArrayList<RPObject> contents2 = new ArrayList<RPObject>(oldSlot2.size());
			ArrayList<RPObject> contents3 = new ArrayList<RPObject>(oldSlot3.size());
			...
			for (RPObject item : oldSlot2) {
				contents2.add(item);
			}
			for (RPObject item : contents2) {
				oldSlot2.remove(item.getID());
				newSlot2.add(item);
			}
			
			for (RPObject item : oldSlot3) {
				contents2.add(item);
			}
			for (RPObject item : contents3) {
				oldSlot3.remove(item.getID());
				newSlot3.add(item);
			}
			...
			player.setFeature("scrollbag", false);
			player.setFeature("weaponbag", false);
		}
	}
