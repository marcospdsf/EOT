...
public class StendhalPlayerDatabase {
	...
	private void updateExistingTables(final DBTransaction transaction) throws SQLException {
		...
		if (!transaction.doesColumnExist("character_stats", "finger2")) {
			transaction.execute("ALTER TABLE character_stats ADD COLUMN (finger2 VARCHAR(32));", null);
		}
		
		if (!transaction.doesColumnExist("character_stats", "glove")) {
			transaction.execute("ALTER TABLE character_stats ADD COLUMN (glove VARCHAR(32));", null);
		}
