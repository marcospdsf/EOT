...
public class PlayerTransformer implements Transformer {
	...
	void loadItemsIntoSlots(final Player player) {
		...
		// load items
		final String[] slotsItems = { "bag", "rhand", "lhand",
		 "head", "armor", "legs", "feet", "finger", 
		 "finger2", "glove", "cloak", "back", "belt", 
		 "keyring", /*"portfolio",*/ "scrollbag","weaponbag", 
		 "trade", "pouch" };
		...
		try {
			for (final String slotName : slotsItems) {
				...
				} else if (slotName.equals("scrollbag")) {
					newSlot = new PlayerScrollbagSlot(slotName);
				} else if (slotName.equals("weaponbag")) {
					newSlot = new PlayerScrollbagSlot(slotName);
